...
routes_group.add_command(list_cmd, aliases=["l"])
...
def show_cmd(
    ...
    data = base_client.get_entity(
        client, ENTITY_COLLECTION.format(lb_uuid=lb_uuid, vhost_uuid=vhost_uuid), uuid
    )
    show_data(data, output)


routes_group.add_command(show_cmd, aliases=["get", "g"])
...
routes_group.add_command(delete_cmd, aliases=["d"])
