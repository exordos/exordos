...
ENTITY = "instance"
ENTITY_COLLECTION = c.S3_INSTANCE_COLLECTION
FIELDS_MAP = {
    "UUID": "uuid",
    "Project": "project_id",
    "Name": "name",
...
@click.option(
    ...
    type=str,
    required=True,
    help="UUID or name of the s3 version",
)
@click.option(
    "--cpu",
...
def add_cmd(
    ...
    if uuid is None:
        uuid = sys_uuid.uuid4()
    version_uuid = base_client.get_entity(client, c.S3_VERSION_COLLECTION, version)[
        "uuid"
    ]
    data = {
        "uuid": str(uuid),
        ...
        "name": name,
        "version": f"{c.S3_VERSION_COLLECTION}{version_uuid}",
        "cpu": cpu,
        "ram": ram,
        "disk_size": disk_size,
