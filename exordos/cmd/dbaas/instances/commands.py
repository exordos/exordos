...
ENTITY = "instance"
ENTITY_COLLECTION = c.PG_INSTANCE_COLLECTION
FIELDS_MAP = {
    "UUID": "uuid",
    "Project": "project_id",
    "Name": "name",
    ...
    "Disk": "disk_size",
    "Nodes": "nodes_number",
    "Sync replicas": "sync_replica_number",
...
@click.option(
    ...
    type=str,
    required=True,
    help="UUID or name of the postgres version",
)
@click.option(
    "--cpu",
...
@click.option(
    ...
    default=None,
    help="Number of synchronous replicas",
)
def add_cmd(
    ctx: click.Context,
    ...
    disk_size: int,
    nodes_number: int,
    sync_replica_number: int | None,
) -> None:
    client = base_client.get_user_api_client(ctx.obj.auth_data)
    if uuid is None:
        uuid = sys_uuid.uuid4()
    version_uuid = base_client.get_entity(client, c.PG_VERSION_COLLECTION, version)[
        "uuid"
    ]
    data = {
        "uuid": str(uuid),
    ...
    if sync_replica_number is not None:
        data["sync_replica_number"] = sync_replica_number
