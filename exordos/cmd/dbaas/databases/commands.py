...
ENTITY = "database"
ENTITY_COLLECTION = c.PG_DATABASE_COLLECTION
FIELDS_MAP = {
    "UUID": "uuid",
    "Project": "project_id",
    "Name": "name",
...
databases_group = create_entity_group(
...
@click.option(
    ...
    type=str,
    required=True,
    help=f"UUID or name of the user owning the {ENTITY}",
)
def add_cmd(
    ctx: click.Context,
...
databases_group.add_command(add_cmd, aliases=["a"])
databases_group.add_command(update_cmd, aliases=["u"])
