...
from exordos.cmd.auth import qr
...
@click.command("enable-otp", help="Enable two-factor authentication")
@click.option(
    "-p",
    "--password",
    default=None,
...
@click.pass_context
def enable_otp_cmd(ctx: click.Context, password: str | None) -> None:
    client = base_client.get_user_api_client(ctx.obj.auth_data)
    ...
    otp_uri = (result or {}).get("otp_uri")
    if not otp_uri:
        raise click.ClickException("No OTP URI returned from server")
    ...
    click.secho(f"Two-factor authentication enabled for user {user_uuid}", fg="green")


auth_group.add_command(enable_otp_cmd)
