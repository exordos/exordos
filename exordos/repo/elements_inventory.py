...
def merge_inventories(inventory_files):
    ...
    for file_path in inventory_files:
        try:
            ...
            if latest_name_version.get(data["name"]) is None or data[
                "version"
            ] > latest_name_version.get(data["name"]):
                latest_name_version[data["name"]] = data["version"]
...
def build(repository_dir: str | Path, elements_dir: str) -> None:
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python elements_inventory.py <repository_dir>")
        sys.exit(1)

    repository_dir = sys.argv[1]

    for elements_dir in ELEMENTS_DIRS:
        build(repository_dir, elements_dir)
