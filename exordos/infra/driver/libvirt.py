...
class LibvirtInfraDriver(base.AbstractInfraDriver):
    ...
    def _domain2node(self, domain: minidom.Document) -> models.Node:
        cores = int(_get_tag_value(domain, vc.EXORDOS_META_CPU_TAG))
        ram = int(_get_tag_value(domain, vc.EXORDOS_META_MEM_TAG))
        name = _get_tag_value(domain, "name")
        image = _get_tag_value(domain, vc.EXORDOS_META_IMAGE_TAG)
    ...
    def _extract_net_from_bootstrap(
        ...
        net = bootstrap.getElementsByTagName(vc.EXORDOS_META_NET_TAG)[0]
    ...
    def _extract_boot_net_from_bootstrap(
        ...
        try:
            net = bootstrap.getElementsByTagName(vc.EXORDOS_META_BOOT_NET_TAG)[0]
        except Exception:
    ...
    def list_stands(self) -> tp.List[models.Stand]:
        ...
        for d in libvirt.list_xml_domains():
            ...
            stand_name = _get_tag_value(domain, vc.EXORDOS_META_STAND_TAG)
            ...
            node_type = _get_tag_value(domain, vc.EXORDOS_META_NODE_TYPE_TAG)
    ...
    def create_stand(
        ...
        **extra_data: tp.Any,
        ...
        for node in stand.baremetals:
            tags = (
                self._tag(vc.EXORDOS_META_STAND_TAG, stand.name),
                self._tag(vc.EXORDOS_META_CPU_TAG, node.cores),
                self._tag(vc.EXORDOS_META_MEM_TAG, node.memory),
                self._tag(vc.EXORDOS_META_NODE_TYPE_TAG, "baremetal"),
            )
